...
{
    public class InsanityLotteryTest
    {
        private readonly ITestOutputHelper output;

        public InsanityLotteryTest(ITestOutputHelper output)
        {
            this.output = output;
        }
        ...
        private void RunTests(long maxValue, Tile expected)
        {
            ...
            var subject = new InsanityLottery();

            var nope = 0;

            for (var i = 0; i < testCount; i++)
            {
                subject = (InsanityLottery)subject.CreateNext();
